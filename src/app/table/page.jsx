...
function OrderHistoryComponent() {
  ...
  useEffect(() => {
   ...
   const showOrderDetails = (order) => {
    // Membuat string HTML untuk daftar items dan total price
    ...
    const totalPrice = (order.items.reduce((total, item) => total + item.price * item.qty, 0) * 1.05).toLocaleString();
    
    // Menampilkan SweetAlert dengan daftar item dan total price
    Swal.fire({
      title: 'Order Details',
      html: `
      ...
      <div className="flex flex-wrap gap-2">
        {filteredOrders.length > 0 ? (
          filteredOrders.map((order, index) => {
            const initial = order.customerData.name ? order.customerData.name.substring(0, 2).toUpperCase() : '-';
            return (
              <div key={index} className="relative w-[15rem] flex flex-col text-center border p-4 rounded-xl mb-2 bg-green-100/40">
                <div className="flex flex-col">
                  <p className="font-bold border-green-600 border rounded-full absolute px-2">
                    {index + 1}
                  </p>
                  <p className="mt-5 text-7xl bg-green-600 mb-2 text-white font-semibold border rounded-full py-12 mx-4 border-green-500">{initial}</p>
                  <p><strong className='text-neutral-800'>Customer:</strong> {order?.customerData?.name || "No Name"}</p>
                </div>
